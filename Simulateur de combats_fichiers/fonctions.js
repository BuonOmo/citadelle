...
var TotalUnites = new Array(); //=[nombre d'unités en vie de l'attaquant, nombre d'unités en vie du défenseur]
var TotalUnitesParType = new Array(); //UnitesParType[adv][type]=nombre d'unités en vie et pas en duel de type type de l'adv
var nbrUniteEnForme = new Array(); //nbrUniteEnForme[adv][type]=nombre d'unités en forme de type type de l'adv
var Unites = new Array(); //Unites[adv][type][n] = PV de la nième unité de type type de l'adv. (tableau trié ainsi : [en duel, ..., en duel, blessé, blessé, ..., blessé])
var Duels = new Array(); //Duels[n][adv][info] = info de l'unité de l'adv du duel numéro n.
...
{
    Initialisation();
    var adv, type, n, degats, Maj, i, j, gagnant, Moyenne=new Array(), len, EType=new Array(), StrGagnant;
    var Chrono=0; //temps que dure la bataille
    var nbrDuels=CountDuels();
    for (n=0; n<nbrDuels; n++) {ChooseDuels();}
    while(nbrDuels>0)
    {
        ...
        for (i=0; i<nbrDuels; i++) //pour chaque duel
        {
			for (adv=0; adv<2; adv++)
			{
				...
				if(Duels[i][adv][0]==1+adv && Duels[i][1-adv][0]!=8) {degats += 4;} // cata (ou baliste) vs !AM => 5 ou 6
				else if (Duels[i][adv][0]==3 && (Duels[i][1-adv][0]==4 || Duels[i][1-adv][0]==7)) {degats += 3;} //piquier vs cavalier ou chevalier
				else if (Duels[i][adv][0]==4 && Duels[i][1-adv][0]==5) {degats += 2;} //cavalier vs archer
				else if (Duels[i][adv][0]==5 && (Duels[i][1-adv][0]==3 || Duels[i][1-adv][0]==6)) {degats += 2;} //archer vs piquier ou paladin
				else if (Duels[i][adv][0]==7 && Duels[i][1-adv][0]==0) {degats +=2;} //chevalier vs soldat
		...
		if (Maj.length!=0)
		{
			...
			{
				if (j<len && i+j==Maj[j]) {j++; i--;}
				else {Duels[i]=Duels[i+j];}
			}
			for (i=0 ; i<len; i++) Duels.pop(); //on supprime les duels inutiles
			nbrDuels=CountDuels();
			for (n=Duels.length; n<nbrDuels; n++) {ChooseDuels();} //on tire les derniers duels
		}
    }
	...
	if(Simulation == 0){ //si on veut afficher
		if(gagnant != 2){
			for (type = 0; type < 9; type++) { //moyenne
				if(Unites[gagnant][type].length!=0){Moyenne.push((sum(Unites[gagnant][type]))/Unites[gagnant][type].length);}
				else{Moyenne.push(0);}
			}
			for (type = 0; type < 9; type++) { //ecart type
				...
				if(Unites[gagnant][type].length!=0){EType.push(Math.sqrt(j/Unites[gagnant][type].length));}
				else EType.push(0);
			}
			if(gagnant==0){StrGagnant="L'attaquant";}else{StrGagnant="Le défenseur";}
...
function Initialisation()
{
	...
	for (i = 0; i < 2; i++) {
		for (j = 0; j < 9; j++) {
			if (TotalUnitesParTypeInput[i][j].value > 0){a=parseInt(TotalUnitesParTypeInput[i][j].value);} else {a = 0;}
...
function CountDuels()
{
	...
	if (TotalUnites[0]>TotalUnites[1]) {A=TotalUnites[0]; a=TotalUnites[1];}
	else {A=TotalUnites[1]; a=TotalUnites[0];}
	...
	if(TotalUnites[0] == 0 || TotalUnites[1] == 0){nbrDuels=0;}
	return nbrDuels;
}
...
function ChooseDuels()
{
	...
	for (adv=0; adv<2; adv++)
	{
		...
		for (type=0; aleat>=TotalUnitesParType[adv][type]; type++) {aleat-=TotalUnitesParType[adv][type];} //on cherche le type de l'unité
		...
		{
			if (TotalUnitesParType[adv][type] > nbrUniteEnForme[adv][type]) {Unites[adv][type].push(Unites[adv][type][nbrdueltype]); Unites[adv][type][nbrdueltype]=UnitesPv[type];}
...
function fCommerce(){
	...
	for (i = 0; i < 2; i++) {
		for (j = 0; j < 2; j++) {
			if(com[i][j] > 0){min[i][j]=Math.ceil((com[i][j]/2));}else{min[i][j]=1}
		}
	}
	...
	for (i = 0; i < 2; i++) {
		for (j = 0; j < 2; j++) {
			k = 0;
			if(j==1){k=-(min[1-Math.abs(j-i)][1]-min[1-Math.abs(j-i)][0]);}
